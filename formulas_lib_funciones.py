...
def evaluar_elegibilidad_internacional(edad_tecnica, ente_rector):
  ...
  if ente_rector in entes_internacionales:
    if edad_tecnica < 14:
      return (
          False,
          f"Edad técnica insuficiente ({edad_tecnica} años). Mínimo requerido:"
          " 14 años.",
      )
  return True, None
...
def calcular_fecha_alerta(fecha_inicio_competencia, dias_anticipacion=15):
  if isinstance(fecha_inicio_competencia, str):
    fecha_inicio_competencia = datetime.datetime.strptime(
        fecha_inicio_competencia, "%Y-%m-%d"
    ).date()
  ...
  if idx_pb_absoluto < len(df) - 1:
    ...
    if len(df_posterior) >= 2:
      ...
      for i in range(len(df_posterior) - 1):
        if (
            df_posterior.iloc[i]["Tiempo"] > T_pb
            and df_posterior.iloc[i + 1]["Tiempo"] > T_pb
        ):
...
def dibujar_lineas_referencia(
    ax, ref_data, lim_x_min, lim_x_max, peor_tiempo=None
):
  if not ref_data:
    return
  ...
  for cfg in configs:
    ...
    if val and isinstance(val, (int, float)) and val > 0:
      if (y_min * 0.99) <= val <= (y_max * 1.01):
        ax.axhline(
            y=val, color=cfg["col"], linestyle=":", linewidth=0.8, alpha=0.7
        )
        lbl_texto = (
            f"{cfg['lbl']}: {formatear_a_minutos(val).replace(' s', '')}"
        )
        ax.text(
            x_pos,
            val,
...
# -------------------------------------------------------------
# MOTOR MATEMÁTICO DOBLE CÁLCULO DE CURVA AJUSTADO
# -------------------------------------------------------------
def resolver_k_individual(
    eq_t0, eq_T0, eq_t_pb, eq_T_pb, eq_t_peak, eq_T_target
):
...
def enviar_correo_con_pdf(
  ...
  try:
    ...
    if pdf_bytes:
      ...
      adjunto.add_header(
          "Content-Disposition", "attachment", filename=nombre_archivo_pdf
      )
      msg.attach(adjunto)
