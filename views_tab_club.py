...
def renderizar_tab_club():
    ...
    with subtab_atletas:
        ...
        # Cargar todos los nadadores con sus campos principales
        try:
            res_plantilla = supabase.table("usuarios")\
                .select("id, nombre, usuario, email, estatus, creado_at")\
                .eq("rol", "Nadador")\
                .execute()
        ...
        if df_plantilla.empty:
            ...
            # --- TABLA DE PLANTILLA ---
            cols_p_mostrar = ["nombre", "usuario", "email", "estatus", "creado_at"]
